...
const GymiProviderOverview = ({ gymiProviders, courseDetails }: GymiProviderOverviewProps) => {
    ...
    const toggleModal = (providerId: number, state: boolean) => {
        if (gymiProviders && gymiProviders[providerId]) {
            const selected = gymiProviders[providerId];
            ...
            <div className="flex min-h-screen flex-col items-start">
                ...
                <div className="container my-12 mx-auto">
                    <div className="flex flex-wrap -mx-1 lg:-mx-4">
                        {gymiProviders && gymiProviders.map((provider, index) => {
                                                ...
                                                <div className="relative w-auto my-6 mx-auto max-w-lg w-full">
                                                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                                        ...
                                                        <div className="relative p-6 flex flex-col gap-3 text-sm overflow-y-auto max-h-[60vh]">
                                                            ...
                                                            <p><span className="font-medium">Preis/Woche (Langzeit):</span> {selectedProvider['Preis pro Woche Langzeitkurs'] ? 'CHF ' + selectedProvider['Preis pro Woche Langzeitkurs'] : 'Nicht verfügbar'}</p>
                                                            <p><span className="font-medium">Preis/Std. (Intensiv):</span> {selectedProvider['Preis pro Std. Intensiverkurs'] ? 'CHF ' + selectedProvider['Preis pro Std. Intensiverkurs'] : 'Nicht verfügbar'}</p>
                                                            ...
                                                            <p><span className="font-medium">Experten:</span> {boolLabel(selectedProvider.Experten)}</p>
                                                            <p><span className="font-medium">FAQ:</span> {boolLabel(selectedProvider.FAQ)}</p>
